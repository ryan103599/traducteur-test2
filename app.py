    ...
    for item in items:
        ...
        def cards(group):
            ...
            for f in group:
                name = str(f.get("name", ""))
                p = quote(name, safe="")
                out.append('<div class="file-card"><img src="/api/admin/storage/file?work_id='+wid+'&path='+p+'&preview=1" onclick="window.openImage(this.src);this.focus()" tabindex="0" alt="'+escape(name, quote=True)+'" loading="lazy"><div class="file-name">'+escape(name)+'</div><div class="file-meta">'+escape(str(f.get("size_human", "")))+'</div><div class="file-actions"><a href="/api/admin/storage/file?work_id='+wid+'&path='+p+'">Télécharger</a></div><div class="file-menu"><button type="button" onclick="event.stopPropagation();this.parentElement.classList.toggle(&quot;open&quot;)">⋮</button><div class="file-menu-list" onclick="event.stopPropagation()"><button type="button" data-action="rename" data-id="'+wid+'" data-path="'+escape(name, quote=True)+'" onclick="event.stopPropagation();renameFile(this.dataset.id,this.dataset.path)">Renommer</button><button type="button" data-action="edit-meta" data-id="'+wid+'" onclick="event.stopPropagation();editMeta(this.dataset.id)">Modifier les données</button><button type="button" data-action="delete" data-id="'+wid+'" onclick="event.stopPropagation();removeItem(this.dataset.id)">Supprimer</button></div></div></div>')
            return "".join(out)
...
function render(){
  ...
  jobs.innerHTML=items.map(function(item){
    ...
    function card(file){
      ...
      return '<div class="file-card"><img src="'+preview+'" data-preview="'+preview+'" onclick="window.openImage(this.src)" alt="'+esc(file.name)+'"><div class="file-name">'+esc(file.name)+'</div><div class="file-meta">'+esc(file.size_human)+'</div><div class="file-actions"><a href="'+download+'">Télécharger</a></div><div class="file-menu"><button type="button" onclick="event.stopPropagation();this.parentElement.classList.toggle(&quot;open&quot;)">⋮</button><div class="file-menu-list" onclick="event.stopPropagation()"><button type="button" data-action="rename" data-id="'+esc(item.id)+'" data-path="'+esc(file.name)+'" onclick="event.stopPropagation();var id=this.dataset.id,path=this.dataset.path,old=path.split(&quot;/&quot;).pop(),name=prompt(&quot;Nouveau nom du fichier :&quot;,old);if(name&amp;&amp;name!==old){fetch(&quot;/api/admin/storage/&quot;+encodeURIComponent(id)+&quot;/rename&quot;,{method:&quot;POST&quot;,headers:{&quot;Content-Type&quot;:&quot;application/json&quot;},body:JSON.stringify({path:path,name:name})}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error||&quot;Renommage impossible.&quot;);});}).then(function(){location.reload();}).catch(function(e){alert(e.message);});}return false;">Renommer</button><button type="button" data-action="edit-meta" data-id="'+esc(item.id)+'" onclick="event.stopPropagation();var id=this.dataset.id,ip=prompt(&quot;Adresse IP :&quot;,&quot;&quot;);if(ip!==null){var date=prompt(&quot;Date et heure (AAAA-MM-JJTHH:MM) :&quot;,&quot;&quot;);if(date!==null){fetch(&quot;/api/admin/storage/&quot;+encodeURIComponent(id)+&quot;/metadata&quot;,{method:&quot;POST&quot;,headers:{&quot;Content-Type&quot;:&quot;application/json&quot;},body:JSON.stringify({client_ip:ip,created_at:date})}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error||&quot;Modification impossible.&quot;);});}).then(function(){location.reload();}).catch(function(e){alert(e.message);});}}return false;">Modifier les données</button><button type="button" data-action="delete" data-id="'+esc(item.id)+'" onclick="event.stopPropagation();if(confirm(&quot;Supprimer définitivement ce dossier et toutes ses images ?&quot;)){fetch(&quot;/api/admin/storage/&quot;+encodeURIComponent(this.dataset.id),{method:&quot;DELETE&quot;}).then(function(r){if(!r.ok)throw new Error(&quot;Suppression impossible.&quot;);}).then(function(){location.reload();}).catch(function(e){alert(e.message);});}return false;">Supprimer</button></div></div></div>';
    }
