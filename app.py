...
.empty{padding:30px;text-align:center;color:#667085}.error{padding:14px;background:#fef3f2;color:#b42318;border-radius:10px;margin-top:15px}
.filters{display:flex;gap:8px;flex-wrap:wrap;margin:20px 0}.filter{padding:10px 16px;border:1px solid #d0d5dd;border-radius:10px;background:white;color:#344054;cursor:pointer;font-weight:600}.filter.active{background:#111827;color:white;border-color:#111827}
</style></head><body><div class="card">
...
<div id="error"></div>
<div class="filters">
<button type="button" class="filter active" data-filter="all">Toutes</button>
<button type="button" class="filter" data-filter="translated">Images traduites</button>
<button type="button" class="filter" data-filter="uploaded">Images envoyées</button>
</div>
<div id="jobs"></div>
</div>
<script>
...
function fileUrl(item,file){return "/api/admin/storage/file?work_id="+encodeURIComponent(item.id)+"&path="+encodeURIComponent(file.name);}
function isImage(name){return /\.(jpg|jpeg|png|webp|tif|tiff)$/i.test(name);}
function card(item,file){
  const url=fileUrl(item,file);
  return '<div class="file-card"><img src="'+url+'&preview=1" alt=""><div class="file-name">'+esc(file.name)+'</div><div class="file-meta">'+esc(file.size_human)+'</div><div class="file-actions"><a href="'+url+'">Télécharger</a></div></div>';
}
...
function render(){
  ...
  if(!storageItems.length){
    body.innerHTML='<div class="empty">Aucun fichier temporaire actuellement stocké.</div>';
    return;
  }
  storageItems.forEach(function(item){
    const inputs=item.files.filter(function(file){
      return /^input_\d+\./i.test(file.name) && isImage(file.name);
    });
    const outputs=item.files.filter(function(file){
      return file.name.toLowerCase().indexOf("traduit/")===0 && isImage(file.name);
    });
    const zips=item.files.filter(function(file){return file.name.toLowerCase().endsWith(".zip");});
    ...
    if(currentFilter==="all" || currentFilter==="uploaded"){
      html+='<div class="section"><h3>Images envoyées</h3><div class="file-grid">'+(inputs.length?inputs.map(function(file){return card(item,file);}).join(""):'<div class="muted">Aucune image envoyée.</div>')+'</div></div>';
    }
    if(currentFilter==="all" || currentFilter==="translated"){
...
async function load(){
  try{
    const response=await fetch("/api/admin/storage",{credentials:"same-origin",cache:"no-store"});
    if(!response.ok) throw new Error("Impossible de charger le stockage ("+response.status+").");
    const data=await response.json();
    storageItems=Array.isArray(data.items)?data.items:[];
