...
function render(){
  ...
  jobs.innerHTML=items.map(function(item){
    ...
    function card(file){
      ...
      return '<div class="file-card"><img src="'+preview+'" data-preview="'+preview+'" onclick="window.openImage(this.src)" alt="'+esc(file.name)+'"><div class="file-name">'+esc(file.name)+'</div><div class="file-meta">'+esc(file.size_human)+'</div><div class="file-actions"><a href="'+download+'">Télécharger</a></div><div class="file-menu"><button type="button">⋮</button><div class="file-menu-list"><button type="button" data-action="rename" data-id="'+esc(item.id)+'" data-path="'+esc(file.name)+'">Renommer</button><button type="button" data-action="edit-meta" data-id="'+esc(item.id)+'">Modifier les données</button><button type="button" data-action="delete" data-id="'+esc(item.id)+'">Supprimer</button></div></div></div>';
    }
