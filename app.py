...
import traduit
import os, shutil, tempfile, threading, uuid, zipfile, time, traceback
...
IMAGE_TIMEOUT = 300  # 5 minutes maximum par image

HTML = r'''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Traducteur d'images</title><style>
...
<h1>Traducteur d'images</h1><div class="sub">Traitement automatique d'un dossier d'images</div>
...
async function poll(){try{const r=await fetch('/status/'+job),j=await r.json();if(!r.ok)throw Error(j.error||'Erreur de statut');bar.style.width=(j.percent||0)+'%';status.textContent=j.status||'Traduction…';if(j.preview)after.innerHTML='<img src="'+j.preview+'?t='+Date.now()+'">';if(j.done){go.disabled=false;if(j.error){status.textContent='❌ '+j.error;return}dl.disabled=false;status.textContent='✅ Terminé : '+j.total+' image(s). Clique sur « Télécharger le ZIP ». ';return}setTimeout(poll,500)}catch(e){status.textContent='❌ Erreur de suivi : '+e.message;go.disabled=false}}
...
def new_job(target,total):
    jid=uuid.uuid4().hex
    with LOCK:
        JOBS[jid]={"target":target,"percent":0,"current":0,"total":total,"done":False,
                   "status":"Préparation…","error":None,"download":None,"preview":None}
    return jid
...
def run_folder(jid,files,target):
    ...
    try:
        ...
        for i,(filename,data) in enumerate(files,1):
            ...
            with LOCK:
                JOBS[jid].update(current=i,status=f"Image {i}/{total} • chargement OCR…")
            try:
                traduit.translate_image(src,dest,target)
            except Exception as e:
                traceback.print_exc()
                raise RuntimeError(f"Image {i}/{total} : erreur pendant OCR/traduction : {type(e).__name__}: {e}") from e
            elapsed=time.monotonic()-started
            if elapsed > IMAGE_TIMEOUT:
                raise RuntimeError(f"Image {i}/{total} : traitement trop long ({int(elapsed)} s). Le traitement a été arrêté.")
            p=int(i*100/total)
            with LOCK:
                JOBS[jid].update(percent=p,current=i,status=f"Image {i}/{total} terminée • {int(elapsed)} s")
                if i==1:JOBS[jid]["preview"]="/result/"+jid
        with LOCK:
            JOBS[jid].update(status="Création du ZIP…")
